...
// ============================================================
//  App state — all in one place, set ONCE at end of init
// ============================================================
static EGLDisplay g_eglDisplay = EGL_NO_DISPLAY;
static EGLSurface g_eglSurface = EGL_NO_SURFACE;
static EGLContext g_eglContext  = EGL_NO_CONTEXT;
static bool g_engineReady = false;   // single flag, set last
...
public:
    ...
    Vector3 GetRight() const {
        return GetFwd().Cross({0,1,0}).Normalized();
    }

private:
    void ApplyRot() {
        if (go) go->GetTransform()->SetEulerAngles({pitch, yaw, 0});
    }
};
...
static void RenderScene(Scene* scene, EditorCamAndroid& edCam, GameObject* selected) {
    ...
    scene->ForEach([&](GameObject* go){
        ...
        glUniform3f(glGetUniformLocation(g_phong,"u_albedo"),
            mat.albedo.r, mat.albedo.g, mat.albedo.b);
        glUniform3f(glGetUniformLocation(g_phong,"u_specular"),
            mat.specular.r, mat.specular.g, mat.specular.b);
        glUniform1f(glGetUniformLocation(g_phong,"u_shininess"), mat.shininess);
        glUniform3f(glGetUniformLocation(g_phong,"u_cameraPos"),
            camPos.x, camPos.y, camPos.z);
        glUniform3f(glGetUniformLocation(g_phong,"u_ambientColor"),
            g_ambient.x, g_ambient.y, g_ambient.z);
        glUniform3f(glGetUniformLocation(g_phong,"u_lightDir"),
            lDir.x, lDir.y, lDir.z);
        glUniform3f(glGetUniformLocation(g_phong,"u_lightColor"),
...
static void BuildDemoScene(Scene* scene) {
    ...
    sl->SetColor({1.f,0.92f,0.80f,1.f});
    sl->SetIntensity(1.1f);
    ...
    fl->SetColor({0.4f,0.5f,0.8f,1.f});
    fl->SetIntensity(0.4f);
...
static bool InitEGL(ANativeWindow* window) {
    ...
    EGLConfig cfg;
    EGLint numConfigs=0;
    if (!eglChooseConfig(g_eglDisplay, attribs, &cfg, 1, &numConfigs) || numConfigs==0) {
    ...
    g_eglSurface = eglCreateWindowSurface(g_eglDisplay, cfg,
...
static void InitEngine(ANativeWindow* window) {
    ...
    LOGI("Demo scene built");

    g_edCam.Init(g_scene, (float)g_vpW / (float)g_vpH);
    LOGI("Camera OK");

    // Set flag LAST — only now is it safe to render/handle input
    g_engineReady = true;
    LOGI("Nexus Engine ready!");
}
...
static int32_t HandleInput(android_app* app, AInputEvent* event) {
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        if (action==AMOTION_EVENT_ACTION_DOWN && !inUI) {
            ...
            g_edCam.OnTouchDrag(tx-g_lastTX, ty-g_lastTY);
...
void android_main(android_app* app) {
    ...
    while (true) {
        ...
        int timeout = g_engineReady ? 0 : -1;
        int result = ALooper_pollOnce(timeout, nullptr, &events, (void**)&source);
