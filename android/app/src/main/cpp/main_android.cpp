...
//  Full port of the desktop engine to Android + OpenGL ES 3
// ============================================================
...
// ============================================================
//  OpenGL ES 3 Shaders  (#version 300 es)
// ============================================================
...
static void SetMat4(GLuint prog, const char* n, const Matrix4& m) {
...
static void BuildGrid() {
    int half=20; float spacing=1.0f;
    std::vector<float> verts;
    float ext=half*spacing;
    for (int i=-half;i<=half;++i){float z=i*spacing;verts.insert(verts.end(),{-ext,0,z,ext,0,z});}
    for (int i=-half;i<=half;++i){float x=i*spacing;verts.insert(verts.end(),{x,0,-ext,x,0,ext});}
...
static bool InitRenderer(int w, int h) {
    ...
    if (!g_phong||!g_unlit) return false;
    BuildGrid();
...
// ============================================================
//  Editor Camera (touch-friendly, no GLFW)
// ============================================================
...
public:
    ...
    float lastTX=0,lastTY=0; bool dragging=false;
...
private:
    void ApplyRot(){ go->GetTransform()->SetEulerAngles({pitch,yaw,0}); }
};
...
static void RenderScene(Scene* scene, EditorCamAndroid& edCam, GameObject* selected) {
    ...
    scene->ForEach([&](GameObject* go){
        ...
        if (l&&l->IsEnabled()) lights.push_back(l);
    });
    ...
    scene->ForEach([&](GameObject* go){
        ...
        for (Light* l:lights)
            if (l->GetType()==LightType::Directional){lDir=l->GetDirection();lCol=l->GetEffectiveColor();lInt=l->GetIntensity();break;}
...
static void BuildDemoScene(Scene* scene) {
    ...
    sl->SetColor({1.f,0.92f,0.80f}); sl->SetIntensity(1.1f);
    ...
    fl->SetColor({0.4f,0.5f,0.8f}); fl->SetIntensity(0.4f);
...
// ============================================================
//  Editor UI (ImGui, touch-friendly)
// ============================================================
static void DrawEditorUI(Scene* scene, EditorCamAndroid& edCam,
    ...
    edCam.moveF |= ImGui::Button("^##mf",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SetCursorPos(ImVec2(0,bsz));
    edCam.moveL |= ImGui::Button("<##ml",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SameLine();
    edCam.moveD |= ImGui::Button("v##md",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SameLine();
    edCam.moveR |= ImGui::Button(">##mr",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SetCursorPos(ImVec2(0,bsz*2));
    edCam.moveU |= ImGui::Button("Up##mu",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SameLine();
    edCam.moveB |= ImGui::Button("Dn##mb",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::End();
    ...
    ImGui::Text("LOOK");
    ImGui::SetCursorPos(ImVec2(bsz,5));
    edCam.lookU |= ImGui::Button("^##lu",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SetCursorPos(ImVec2(0,bsz));
    edCam.lookL |= ImGui::Button("<##ll",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SameLine();
    edCam.lookD |= ImGui::Button("v##ld",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::SameLine();
    edCam.lookR |= ImGui::Button(">##lr",ImVec2(bsz,bsz))&&ImGui::IsItemActive();
    ImGui::End();
}
...
static bool InitEGL(ANativeWindow* window) {
    ...
    eglChooseConfig(g_eglDisplay,attribs,&cfg,1,&n);
...
static void HandleCmd(android_app* app, int32_t cmd) {
    switch (cmd) {
        case APP_CMD_INIT_WINDOW:
            if (app->window) InitEngine(app->window);
            break;
...
void android_main(android_app* app) {
    ...
    while (true) {
        ...
        if (g_initialized&&g_rendererReady) RenderFrame();
    }
    ShutdownEngine();
}
